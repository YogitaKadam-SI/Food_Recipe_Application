    ...
    companion object{
        ...
        fun onRecipeClickListener(recipeRowLayout : ConstraintLayout , result: Result){
            ...
            recipeRowLayout.setOnClickListener {
                try {
                    val action =
                        RecipiesFragmentDirections.actionRecipiesFragmentToDetailsActivity(result)
...
imageView.load(imageUrl){
    ...
    error(R.drawable.ic_error_placeholder)
        ...
        @JvmStatic
        fun parseHtml(textView: TextView, desccription: String?){
            if(desccription != null) {
                val desc = Jsoup.parse(desccription).text()
                textView.text = desc
            }
        }
